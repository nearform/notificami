...
We have multiple packages, but there are 3 core building blocks:

### notifications-backend-core

This package contains the basic functionality to store and send notifications.
...
This object has the responsibility to store and map notification to some sort of storage. It also can freely decide the structure of the notification object.
In our default implementation (postgres), we have used a structure described below when discussing the hapi plugin HTTP REST API responses.
...
To change the server configuration, create a custom configuration file and put its path as the value of the `NF_NOTIFICATION_CONFIG_PATH` env variable. We will use that file configuration to build our server instead of our default.

The custom config file should be like the following:
...
While building the notification server, hapi-plugin and core modules, we found that a plugin approach was the easiest one to allow users to define their own notification system. To understand what you can do by adding plugins to the server, we will describe our 2 default plugins.

### notifications-backend-test-queue

This plugin will install a queue implementation to send and consume notifications.

The code is very easy, but its worth explaining each section.

First of all we need to listen for the `add` event, triggered every time a notification is stored. To do so we use the `notificationsService` property of the `server` object. This property is set by the `notifications-backend-hapi-plugin`.
...
We will also add a consumer on the same server (usually not a good idea, but this is an example ;) ). to actually send the notification we reach again for the `notificationsService` property on the `server` and call its `send` method.
...
You can find the whole code [here](https://github.com/nearform/notifications/tree/master/packages/notifications-backend-test-queue)
...
- it register its own server method `notificationsChannelWebsocketNesNotifyUser`
