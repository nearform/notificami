...
describe('Notification - getByUserIdentifier', () => {
  ...
  describe('getByUserIdentifier', () => {
    test('should correctly return the notification list related to the user', async () => {
      ...
      expect(result[0].notify.content).to.be.equal('Some notification content 9')
      expect(result[1].notify.content).to.be.equal('Some notification content 8')
      expect(result[2].notify.content).to.be.equal('Some notification content 7')
      expect(result[3].notify.content).to.be.equal('Some notification content 6')
      expect(result[4].notify.content).to.be.equal('Some notification content 5')

      result = await this.notificationsService.getByUserIdentifier('filippo')
      expect(result.length).to.be.equal(2)
      expect(result[0].notify.content).to.be.equal('Some notification to Filippo 2')
      expect(result[1].notify.content).to.be.equal('Some notification to Filippo 1')
    ...
    test('should correctly return the notification list related to the user with a different limit', async () => {
      ...
      expect(result[3].notify.content).to.be.equal('Some notification content 6')
      expect(result[4].notify.content).to.be.equal('Some notification content 5')
      expect(result[5].notify.content).to.be.equal('Some notification content 4')
    })
    ...
    test('set a offsetId it should return the list of the notifications older than the ID', async () => {
      ...
      expect(result[0].notify.content).to.be.equal('Some notification content 4')
      expect(result[1].notify.content).to.be.equal('Some notification content 3')
      expect(result[2].notify.content).to.be.equal('Some notification content 2')
      expect(result[3].notify.content).to.be.equal('Some notification content 1')
    })
  })
})
